...
import type { CreateTravelRequestTripCommand } from './create-travel-request.use-case';
...
export class CorrectRejectedTravelRequestTripUseCase {
  ...
  async execute(
    ...
    const departureDate = parseDateOrThrow(trip.fechaSalida, 'fechaSalida del viaje');
    const returnDate = parseDateOrThrow(trip.fechaRegreso, 'fechaRegreso del viaje');
    ...
    const gasolinaTotal = trip.gasolina.necesitaGasolina
      ...
      : 0;
    const tagTotal = trip.tag.necesitaTag ? toSafeNumber(trip.tag.montoSolicitado) : 0;
    const totalEstimado = roundToTwo(gastosTotal + gasolinaTotal + tagTotal);

    if (trip.gasolina.necesitaGasolina && trip.gasolina.cardId) {
      const card = await this.travelRequestRepository.findFuelCardById(trip.gasolina.cardId);

      if (!card || card.type !== 'FUEL' || !card.isActive) {
        throw new BadRequestException('La tarjeta de gasolina no es válida o está inactiva.');
      }
    }
    ...
    const solicitud = await this.travelRequestRepository.findTravelRequestDetailByTripForUser(
      command.tripId,
      command.userId,
    );
